  ...
  const joinRoom = useCallback(
    async (roomCode) => {
      ...
      try {
        ...
        setRoom(data[0]);
        toast.success(`Berhasil bergabung ke room ${roomCode}!`, {
          id: toastId,
        });
      } catch (error) {
        toast.error(error.message, { id: toastId });
      ...
      <div className="main-content">
        <div className="page-header">
          ...
          <p className="page-subtitle">
            Ruang:{" "}
            <span className="font-bold text-teal-600">{room.room_code}</span>
          </p>
        </div>
        {isDragging && (
...
export default function LajuApp() {
  ...
  useEffect(() => {
    ...
    console.log(
      "CLIENT-SIDE Supabase Key Loaded:",
      process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY ? "ADA" : "TIDAK ADA / KOSONG"
    );
  }, []);

  const { room, connectionStatus, createRoom, joinRoom, cancelRoom } =
    useRoomManagement(clientId);
  ...
  if (!clientId) {
    ...
    ); // Loading state until clientId is ready
  }

  if (room?.status === "connected")
    return <ConnectedRoom room={room} connectionStatus={connectionStatus} />;
  ...
  return (
    <div className="app-container">
      <Head>
        ...
        <meta
          name="description"
          content="Transfer file langsung antar perangkat tanpa melalui server secara real-time."
        />
      ...
      <div className="main-content">
        <div className="page-header">
          ...
          <p className="page-subtitle">
            Transfer file super cepat tanpa ribet. Langsung antar perangkat,
            aman dan mudah.
          </p>
        </div>
      ...
      <footer className="footer">
        <p>
          &copy; {new Date().getFullYear()} Laju.io - Dibuat dengan ❤️ untuk
          kemudahan transfer file.
        </p>
      </footer>
    </div>
  );
